...
| Recommendation engine | Rule-based (`app/services/recommendation_service.py`), not ML | **State this plainly wherever the product is described.** The MVP engine is four explicit, explainable rules (unused agent, high cost, likely duplicate, high-risk permission) run over real data. It is not a learned model. Swapping in one later only touches this module — the API and UI don't change. |
